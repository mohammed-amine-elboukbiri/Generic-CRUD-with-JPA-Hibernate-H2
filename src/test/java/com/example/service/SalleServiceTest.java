...
public class SalleServiceTest {
    ...
    public void testCrudOperations() {
        ...
        salle.setDescription_salle("Test description");
        salle.setEtage_salle(1);

        Salle savedSalle = service.save(salle);
        assertNotNull(savedSalle.getId_salle());

        // Read
        Optional<Salle> foundSalle = service.findById(savedSalle.getId_salle());
        assertTrue(foundSalle.isPresent());
        assertEquals("Test Room", foundSalle.get().getNom_salle());

        // Update
        Salle toUpdate = foundSalle.get();
        toUpdate.setCapacite_salle(30);
        service.update(toUpdate);

        Optional<Salle> updatedSalle = service.findById(savedSalle.getId_salle());
        assertTrue(updatedSalle.isPresent());
        assertEquals(Integer.valueOf(30), updatedSalle.get().getCapacite_salle());

        // Delete
        service.delete(updatedSalle.get());
        Optional<Salle> deletedSalle = service.findById(savedSalle.getId_salle());
    ...
    public void testFindByDisponible() {
        ...
        s1.setDisponible_salle(true);

        Salle s2 = new Salle("Unavailable Room", 30);
        s2.setDisponible_salle(false);
        ...
        assertTrue(availableRooms.stream().anyMatch(s -> s.getNom_salle().equals("Available Room")));
        assertFalse(availableRooms.stream().anyMatch(s -> s.getNom_salle().equals("Unavailable Room")));

        List<Salle> unavailableRooms = service.findByDisponible(false);
        assertTrue(unavailableRooms.stream().anyMatch(s -> s.getNom_salle().equals("Unavailable Room")));
    ...
    public void testFindByCapaciteMinimum() {
        ...
        assertEquals(2, roomsMin50.stream()
                .filter(s -> s.getNom_salle().equals("Medium Room") || s.getNom_salle().equals("Large Room"))
                .count());

        List<Salle> roomsMin80 = service.findByCapaciteMinimum(80);
        assertEquals(1, roomsMin80.stream()
                .filter(s -> s.getNom_salle().equals("Large Room"))
                .count());
